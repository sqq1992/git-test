<template>
	<div class="jd-coupon-box" v-loading="createLoading">
		<el-form :label-position="labelPosition" :rules="rules" ref="couponRef" label-width="200px" :model="couponData" v-if="couponData">
...
<!--			</el-form-item>-->
      <el-form-item label="优惠卷信息" class="label-title" />
			<el-form-item
        label="礼品名称："
        prop="giftName"
      >
				<el-input class="w300" :disabled="isDisabledJdCont"  auto-complete="off" v-model="couponData.giftName"></el-input>
				<span>{{couponData.giftName.length}}/10</span>
...
<!--			</el-form-item>-->
			...
			<el-form-item label="礼品介绍：">
				 <el-input :disabled="isDisabledJdCont" type="textarea" class="couponDesc" v-model="couponData.giftIntro"></el-input>
      ...
      <div v-if="isZiying">
        <el-form-item
          ...
          prop="batchKey"
        >
          <el-input class="w300" :disabled="isDisabledJdCont" auto-complete="off" v-model="couponData.batchKey"></el-input>
        </el-form-item>
      </div>
      <div v-if="!isZiying">
        <el-form-item
        ...
        >
          <el-form-item
            label="满"
            prop="couponRequireValue"
            class="inner-form mgr8"
          >
            <el-input :disabled="isDisabledJdCont" type="number" class="" v-model="couponData.couponRequireValue"></el-input>
          </el-form-item>
          <el-form-item
            label="减"
            prop="couponValue"
            class="inner-form"
          >
            <el-input :disabled="isDisabledJdCont" type="number" class="" v-model="couponData.couponValue"></el-input>
        ...
        <el-form-item
          ...
          prop="couponQuantity"
        >
          <div class="inline-block mgr4">
            <el-input :disabled="isDisabledJdCont" step="1" class="" v-model="couponData.couponQuantity"></el-input>
          </div>
          <span class="inline-block">张</span>
        </el-form-item>
        <el-form-item
          label="优惠券面额有效期："
          prop="couponValidity"
        >
          <div class="inline-block mgr4">
            <el-input :disabled="isDisabledJdCont" type="number" class="" v-model="couponData.couponValidity"></el-input>
          </div>
          <span class="inline-block">天</span>
        </el-form-item>
      </div>

      <el-form-item label="兑换设置"  class="label-title"/>
			<el-form-item label="上下架时间：">
				...
				<el-date-picker
					...
					:clearable = false
					:disabled="couponData.status == 'invalid' || couponData.status == 'valid'"
					type="datetime"
					placeholder="选择日期"
        ...
        >
          ...
          <el-radio :label="0">
			...
			<el-form-item label="兑换所需积分：">
				<el-input class="w150"  :disabled = 'giftStatus' auto-complete="off" @blur="blurInp('needPoint')"  v-model="couponData.needPoint"></el-input>
			</el-form-item>
...
import {YYYYMMDDhhmmss} from "../../../config";
	export default{
		data() {
			return{
        rules:{
          giftName: [
            ...
            { min: 1, max: 10, message: '礼品名称长度在1到10个字符', trigger: 'blur' }
          ],
          couponQuantity: [
            {
              required: true,
          ...
          batchKey: [
            { required: true, message: 'batchKey不得为空!', trigger: 'blur' },
          ],
        },
				labelPosition: 'right',
				isUnified: true,
				...
				couponData: {
					...
					addCountNum: '1',
					giftIntro: '',
					upTime: moment().add(5,'minutes').format(YYYYMMDDhhmmss),
					downTime: moment().add(1,'months').endOf('day').format(YYYYMMDDhhmmss),
					needLevel: '',
					allCount: '1',
					needPoint: '1',
					needPointLevel1: '1',
      ...
      isCouldSubmit(){
        ...
        if(!this.isZiying){ // pop店的提交限制
          if(Number(couponData.couponRequireValue)<=Number(couponData.couponValue)){
            this.$message.error('优惠券的满元必须大于等于减元!');
            return false;
          }

          if(Number(couponData.allCount)>Number(couponData.couponQuantity)){
            this.$message.error('总库存总量不能大于优惠券数量!');
            return false;
          }
        }else {   //自营店的提交限制
      ...
      createAct() {
        this.$refs['couponRef'].validate((valid,b) => {
          if (valid && this.isCouldSubmit()) {
            this.submitCb();
          } else {
            return false;
          }
      ...
      submitCb(){
        ...
        ajax({
          ...
          vm.isDisabledJdCont = result.data.status == 'valid' || result.data.status == 'invalid' || result.data.status == 'wait';
