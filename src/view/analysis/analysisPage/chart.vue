...
export default {
  ...
  methods:{
    ...
    initCharts() {
      ...
      var listNameTwo = data.list[1] ? (data.type + data.list[1].value) : "";
      ...
      list.forEach(item => {//返回带%的字符串 echarts无法解析 需要手动转为数值
        (item[listNameOne] + '').indexOf('%') > -1 && (item[listNameOne] = item[listNameOne].slice(0,-1) * 100);
        (item[listNameTwo] + '').indexOf('%') > -1 && (item[listNameTwo] = item[listNameTwo].slice(0,-1) * 100);
      })
      var option = {
        color: colors,
        tooltip: {
