    ...
    <el-table
     ...
     <el-table-column label="活动状态" min-width="120">
      ...
      <el-table-column label="操作" min-width="120">
        <template scope="scope">
          <!-- <Buttons v-bind="scope" :type="scope.row.type" :remark="scope.row.statusRemark"  @click="handleOperation"/> -->
          <mobileButtons v-bind="scope" @click="handleOperation"></mobileButtons>
        </template>
      </el-table-column>
  </el-table>
  ...
  export default{
    ...
    methods: {
      ...
      handleOperation(type,data) {
        let api_name;
        if(type == 'reCreate') {
          this.$router.push({name:'marketingMobile',query:{id:data.id,name: data.name,extra: data.extra,sendTime: data.sendTime}})
