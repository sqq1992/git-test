<template>
    <span>
      ...
      <el-button v-if="canShow('download')" type="text" @click="download">下载发送号码</el-button>
...
<script>
  ...
  import {JdMarketingTypeMap} from "@/config";
  export default {
    props:{
      row: Object,
      $index: Number,
    ...
    methods: {
      ...
      //waiting-待提交，commited-已提交，jd_ready-京东准备开始，jd_sending-京东执行中，jd_sended-京东执行结束，jd_error-京东错误，failed-发送失败,cancel-已取消
        ...
        switch(type) {
          ...
          case 'reCreate':
            ...
            break;
          case 'analysis':
            return status == 'jd_sended' && !this.isZiying;
            break;
          case 'agagin':
            return ['jd_sended','jd_error','failed'].includes(status);
            break;
          case 'statusRemark':
            return !!statusRemark;
            break;
      ...
      download() {
        window.open(`/jdMarketing/exportManualSendLog?activityId=${this.row.id}&exportType=excel`);
      },
      handleAnalysis() {
