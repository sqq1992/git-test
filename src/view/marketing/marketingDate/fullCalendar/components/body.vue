<template>
  <div class="full-calendar-body">
    ...
    <div class="dates" ref="dates">
      <div class="dates-bg">
        <div class="week-row clearfix" v-for="week in currentDates">
          <div class="day-cell" v-for="day in week"
            @click.stop="dayClick(day.date, $event)"
            :class="{'today' : day.isToday,
              'not-cur-month' : !day.isCurMonth}">
            <p class="day-number">{{day.monthDay}}日</p>
            <div v-if="day.show" class="dayDetail">
              <!-- <div style="text-align:center;">{{day.events[0].date.slice(0,10)}}</div>  -->
              <ul v-if="day.events" class="eventUl">
                <li v-for="item in day.events" :title="item.taskName">
                  {{item.taskName}}
                </li>
              </ul>
              <span v-else>当日无营销任务</span>
            </div>
            <ul v-if="day.events" class="eventUl">
              <li v-for="item in day.events" :title="item.taskName">
                {{item.taskName}}
    ...
    methods : {
      ...
      slotEvents (date) {
        ...
        for(let key in this.eventList) {
          let fKey = key.replace(/-(0)?\d/g,function($1,$2){return $1.replace($2,'')});
          if(new Date(date).getTime() == new Date(fKey).getTime()){
            thisDayEvents = this.eventList[key];
          }
        }
      ...
      dayClick(day, jsEvent) {
        // let clone = this.currentDates.slice(0);
        this.currentDates = this.currentDates.map(item => {
          item.forEach(itemDay => {
            if(day == itemDay.date) {
              if(!itemDay.show) {
                itemDay.show = true;
              } else {
