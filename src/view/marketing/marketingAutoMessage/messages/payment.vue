...
<!-- 催付提醒 -->
  <div v-loading="loading">
    <div class="btn--box">
      <el-button-group>
        <el-button :type="type == 1 ? 'primary' : 'normal'" @click="type = 1" size="small">催付提醒</el-button>
        <el-button :type="type == 2 ? 'primary' : 'normal'" @click="type = 2" size="small">发送记录</el-button>
      </el-button-group>
    </div>
    <div v-show="type == 1">
      <div class="tips">
        <p>温馨提示：自动对下单未付款买家发送催付短信提醒，如更改提醒设置，则新设置只对保存设置后的订单生效。</p>
        <p>开启催付前请先设置催付触发条件及短信内容，自定义短信内容提交后需3~5分钟审核时间。</p>
      </div>
      <el-form>
        ...
        <el-form-item label="拍下未付款时间：">
          <el-select v-model="paymentData.pay_hour" class="row_line">
            <el-option
              v-for="item in payTimeData"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            ></el-option>
        ...
        <Sms ref="sms" @send="saveActivity" @testSend="testSend" customBtn="true" :tags="['买家姓名']" :tempType="msgType">
      ...
      return new Promise(resolve => {
        ...
        if(this.paymentData.message) {
          if(!this.paymentData.openOrClose) {
            if(this.lastState) {
              this.$confirm('关闭催付提醒可能导致订单因未付款失败，从而导致销售额损失，是否确认关闭？', {
                  type: 'info'
              }).then(() => {
                resolve();
              })
            } else {
              this.$confirm('您开未开启催付提醒，是否开启？', {
                  type: 'info'
              }).then(() => {
